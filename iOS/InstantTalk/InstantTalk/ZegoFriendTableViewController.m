...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onRecevieMessage:) name:kUserMessageReceiveNotification object:nil];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"userChatSegue"])
    {
        ...
        ZegoUser *user = [[ZegoDataCenter sharedInstance].userList objectAtIndex:indexPath.row].user;
        NSString *sessionID = [[ZegoDataCenter sharedInstance] getSessionID:user.userID];
        ...
        else
        {
            ...
            chatController.userList = [NSMutableArray arrayWithArray:[[ZegoDataCenter sharedInstance] getMemberList:sessionID]];
        }
...
//    else if ([segue.identifier isEqualToString:@"friendSelectSegue"])
//    {
...
- (void)onSelectedFriends:(NSArray<ZegoUser *> *)selectedUsers videoSelector:(BOOL)videoSelector
{
    ...
    else
    {
        ...
        NSArray *chatUsers = [NSArray arrayWithArray:selectedUsers];
        if (chatUsers.count == 1)
        {
            ZegoUser *user = chatUsers.firstObject;
            NSString *sessionID = [[ZegoDataCenter sharedInstance] getSessionID:user.userID];
            if (sessionID)
            {
                chatController.sessionID = sessionID;
        ...
        {
            //多人会话，默认为新session
            chatController.chatTheme = [[ZegoSettings sharedInstance] getMessageTitle:chatUsers];
            chatController.userList = [NSMutableArray arrayWithArray:chatUsers];
            [chatController.userList addObject:[[ZegoSettings sharedInstance] getZegoUser]];
        }
